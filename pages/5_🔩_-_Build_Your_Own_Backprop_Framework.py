...
It's fine to store numpy arrays in the `grads` dictionary, but when it comes time to set a tensor's grad attribute, you should use a tensor.""")
    ...
    st.markdown(r"""
And here are some more directed hints:
""")

    # with st.expander("Help - I don't know what the end_grad argument represents. (Note, I need this to be explained better earlier!)"):
#         st.markdown(r"""
# This serves the same purpose as the `grad_out` argument for our backward functions in the previous section.
...
# Remember, at each stage in our backward pass, we're computing a new gradient, and multiplying that by our old gradient (making use of the chain rule). For instance, when we get to `y`, we'll be computing $\frac{dy}{dx}$, and then multiplying this by $\frac{dL}{dy} = 1$ to get $\frac{dL}{dx}$. In this case, $\frac{dL}{dy}$ is our `end_grad` argument.

# If we call `L.backward()`, then we want to populate `x.grad` and `y.grad` with the values $\frac{dL}{dx}$ and $\frac{dL}{dy}$, respectively. 

# The `end_grad` argument for `L` will just be $\frac{dL}{dL} = 1$ (because we will be computing $\frac{dL}{dy}$ and then multiplying this by `end_grad` to get `y.grad` $= \frac{dL}{dy}$). 

# But calling `L.backward()` will mean we also call `y.backward()` (since it is a recursive function), and when we do this, the `end_grad` argument will be $\frac{dL}{dy}$, because we will be computing $\frac{dL}{dx}$ and then multiplying this by `end_grad` to get `x.grad`.
# """)
...
* `args` and `kwargs`, which are also stored in `n.recipe`""")
    ...
    with st.expander("Help - I'm really stuck on implementing this function."):
        st.markdown(r"""
Here's a template for the function, which you can fill in:
